    ...
    <div id="main-container">
      <div id="container">
        ...
        <div class="shopping-lists">
          ...
          >
            <router-link
              v-bind:to="{ name: 'ShoppingListDetail', params: { id:shoppingList.listId } }"
            >{{ shoppingList.listName }}</router-link>

            <button
              class="modify-button"
              v-on:click="removeShoppingList(shoppingList.listId)"
            >Delete</button>
          </div>
        </div>
        ...
        <div class="group-lists">
          <div class="group-list" v-for="group in groups" v-bind:key="group.groupId">
            <router-link
              v-bind:to="{ name: 'Group', params: { id:group.groupId } }"
        ...
        <div class="invite-lists">
          <div
            class="invite-list"
            v-for="sentInvite in sentInvites"
          ...
          <div class="invite-list" v-for="invite in invites" v-bind:key="invite.inviteId">
            {{invite.groupName}}
            <div></div>
            <button
              href="#"
              ...
              <form v-if="invite.inviteId === newInvite.inviteId && inviteCodeForm === true">
                <div class="form-element">
                  ...
                  <input
                    ...
                    v-model="compareCode.compareCode"
                  />
                </div>
...
export default {
  ...
  created() {
    ShoppingListService.viewShoppingListsByAccountId().then(response => {
      ...
      GroupService.viewAllGroupsByUsername(store.state.user.username).then(
        response => {
  ...
  methods: {
    ...
    createShoppingList() {
      if (this.newShoppingList.listName.length > 24) {
        ...
        ShoppingListService.createShoppingList(this.newShoppingList).then(
          response => {
            if (response.status === 200) {
              window.location.reload();
            }
    ...
    leaveGroup(groupId) {
      GroupService.deleteUserFromGroup(store.state.user.id, groupId).then(
        response => {
          if (response.status === 200) {
            window.location.reload();
          }
    ...
    getGroupById(id) {
      let newGroup = GroupService.getGroupByGroupId(id).then(response => {
        if (response.status === 200) {
          return newGroup;
        }
      });
    ...
    inviteCodeInput: function() {
      if (this.compareCode.compareCode == "") {
        ...
        this.compareCode.compareCode.length === 6 &&
        this.compareCode.compareCode != this.newInvite.inviteCode
      ) {
        this.Error = true;
    ...
    getUsernameById(id) {
      GroupService.getUsernameByUserId(id).then(response => {
        console.log("test5");
